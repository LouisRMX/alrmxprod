...
const doc = new Document({
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ children: [
      new TextRun({ text: "alRMX Calculation Bible v3.0", font: "Arial", bold: true, size: 15, color: "0F6E56" }),
    ...
    children: [
      ...
      new Paragraph({ spacing: { after: 30 }, children: [new TextRun({ text: "alRMX Operational Diagnostic Model v3.0 (Governance-grade)", size: 26, font: "Arial" })] }),
      div(),
      ...
      bullet("Recoverable contribution margin: 40-65% of total, achievable in 90 days"),
      bullet("Active constraint: Fleet, Production, or Demand"),
      bullet("Prioritized actions tied to the active constraint"),
      sp(),
      h3("Economic definition of total loss"),
      p("Total loss is contribution margin loss: the difference between achievable and actual (price - variable costs) * volume. It is NOT EBITDA in accounting terms (EBITDA also deducts fixed costs, depreciation is excluded). It is NOT revenue loss (revenue = price * volume, not margin * volume). The model uses contribution margin because it represents the marginal profit from each additional m3 delivered."),
      sp(),
      ...
      p("Independent of which constraint is active. Calculated separately. Designed to avoid overlap with throughput under the current model rules (see Section 5 for overlap controls)."),
      sp(),
      ...
      sp(),
      h3("Total loss"),
      p("Throughput loss and capLeak are constraint-gated: only one is > 0 at a time. They are not summed via max(). The constraint logic ensures mutual exclusivity before this formula is reached."),
      f("totalLoss = throughputLoss + capLeak + rejectMaterial + partialLeak + surplusLeak + breakdownCost"),
      note("Demurrage NOT included (recovery opportunity). Cost-only savings NOT included (demand-constrained). rejectOpportunityCost NOT included (overlaps with throughput)."),
      ...
      new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: [1800, 2600, 2400, 2906],
        rows: [
          ...
          row(["partialLeak", "Trucks running below capacity", "Low: conceptually independent", "Designed as separate under current rules. Partial loads exist regardless of constraint."], [1800, 2600, 2400, 2906]),
          row(["surplusLeak", "Overproduced concrete wasted", "Low: conceptually independent", "Material cost only. Independent of cycle-based throughput."], [1800, 2600, 2400, 2906]),
      ...
      f("surplusLeak = surplusMid * delDay * materialCost * days"),
      f("// ACTIVE: only one of these is > 0 (constraint-gated):"),
      f("throughputLoss = gapMonthlyM3 * contribSafe * seasonalFactor  [Fleet constraint]"),
      f("capLeak = (cap*0.85 - actual) * opH * opD/12 * contribSafe   [Production constraint]"),
      f("// They are mutually exclusive. Never both > 0."),
      sp(),
      f("totalLoss = throughputLoss + capLeak + rejectMaterial + partialLeak + surplusLeak"),
      note("Since throughputLoss and capLeak are mutually exclusive, this is equivalent to: active_constraint_loss + additive_leakage. No max() needed."),
      f("recoveryLo = totalLoss * 0.40"),
      f("recoveryHi = totalLoss * 0.65"),
      ...
      sp(),
      h3("Version control"),
      p("This document is the governing specification of the alRMX diagnostic model. It is version-controlled alongside the codebase. The current version is v3.0."),
      sp(),
      h3("Change control"),
      bullet("Changes to any assumption in Section 2 (Assumption Register) require re-validation of all affected outputs."),
      bullet("Changes to constraint logic (Section 3.3) require re-running all three test scenarios (Fleet, Production, Demand) and verifying consistency."),
      bullet("Changes to the formula appendix must be reflected in calculations.ts. The appendix and code must always match."),
      sp(),
      h3("Comparability"),
      p("Outputs from different model versions are not directly comparable without re-baselining. If the model version changes between a pre-diagnosis and an on-site assessment for the same plant, note this in the report. Historical snapshots (validated_diagnosis JSON) retain their original model version."),
      sp(),
      h3("Audit trail"),
      p("Every assessment stores: validated_diagnosis (JSON snapshot), diagnosis_generated_at (timestamp), diagnosis_schema_version (integer). These allow reconstruction of what the model produced at any point in time."),

      sp(), sp(), div(),
      new Paragraph({ alignment: AlignmentType.CENTER, children: [
        new TextRun({ text: "End of Calculation Bible v3.0", size: 17, font: "Arial", color: "999999", italics: true }),
      ] }),
    ]
  }]
});
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync('C:/Users/lsh29/Desktop/alRMX_Calculation_Bible_v3.docx', buffer);
  console.log('Generated: alRMX_Calculation_Bible_v3.docx');
});
