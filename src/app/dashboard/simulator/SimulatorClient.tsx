...
function dispatchEfficiency(orderToDispatch: string, idleFreq?: string): number {
  const otdMap: Record<string, number> = {
    'Under 10 minutes — very responsive': 0.95,
    '10 to 15 minutes — good': 0.90,
    '15 to 25 minutes — acceptable': 0.82,
    '25 to 40 minutes — slow, likely causing truck idle time': 0.70,
    'Over 40 minutes — major delays, trucks often waiting': 0.55,
  }
  let eff = 0.80 // default
  for (const [key, val] of Object.entries(otdMap)) {
    if (orderToDispatch?.includes(key.split('—')[0].trim())) { eff = val; break }
  }
  // Idle penalty
...
function simulate(
  ...
  // Base inputs from assessment answers
  const plantCap = num(answers.plant_cap, 120)       // m³/hr
  const opHours = num(answers.op_hours, 10)           // hrs/day
  const opDays = num(answers.op_days, 300)             // days/year
  const actualProd = num(answers.actual_prod, 2000)    // m³/month
  const mixerCap = num(answers.mixer_capacity, 7)      // m³/truck
  const rejectPct = num(answers.reject_pct, 3) / 100
  const cementCost = num(answers.cement_cost, 12)

  // Scenario inputs (from sliders)
  ...
  // Contribution proxy: price - cement_cost * 1.6 (benchmark multiplier for total variable cost)
  ...
  // If dispatch efficiency is the real limiter
  if (fleetDaily > prodDaily && effectiveFleetDaily < prodDaily) {
    bottleneck = 'Dispatch'
  }

  // 6. Baseline for comparison
  const baselineDaily = actualProd / 30 // rough monthly to daily
  ...
  // 8. Rough scores
  const prodScore = Math.min(100, Math.round((actualProd / (plantCap * opHours * 30)) * 100 * (utilTarget / 0.92)))
  const fleetScore = Math.min(100, Math.round((totalDeliveries / Math.max(1, num(answers.deliveries_day, 30))) * 70))
  const dispatchScore = Math.round(dispEff * 100)
...
function generateNarrative(
  ...
  if (overrides.utilTarget !== Math.round(num(baseAnswers.actual_prod, 2000) / (num(baseAnswers.plant_cap, 120) * num(baseAnswers.op_hours, 10) * 30) * 100)) {
    changes.push(`plant utilization target to ${overrides.utilTarget}%`)
  }

  if (changes.length === 0) return 'Adjust the sliders above to model improvement scenarios.'

  const changeTxt = changes.join(' and ')
  const volumePct = baseline.annual > 0
    ? Math.round((scenario.incrementalVolume / baseline.annual) * 100)
    : 0

  let txt = `Changing ${changeTxt} could increase annual output by approximately ${fmtVolume(scenario.incrementalVolume)} (+${volumePct}%), equivalent to ${fmtMoney(scenario.revenueUpside)} in additional revenue.`

  if (scenario.bottleneck !== baseline.bottleneck && baseline.bottleneck) {
    txt += ` Under this scenario, the primary bottleneck shifts from ${baseline.bottleneck} to ${scenario.bottleneck}.`
  } else {
    txt += ` ${scenario.bottleneck} remains the primary constraint.`
  }

  // Warning: trucks adding little value
  if (overrides.trucks > baseTrucks && scenario.bottleneck === 'Production') {
    txt += ` ⚠️ Additional trucks have limited impact — production capacity is the binding constraint.`
  }
...
export default function SimulatorClient({ assessments }: SimulatorClientProps) {
  ...
  // Extract baseline values from selected assessment
  const baseAnswers = (selected?.answers || {}) as Record<string, string>

  // Turnaround: parse from answer option text
  ...
  // Reset sliders when assessment changes
  ...
  const narrative = useMemo(() => {
    ...
    return generateNarrative(
      ...
      result,
      { turnaround, trucks, utilTarget, priceM3 },
      baseAnswers
    )
  }, [selected, result, turnaround, trucks, utilTarget, priceM3, baseAnswers])
  ...
  const sliderStyle = {
    ...
    appearance: 'none' as const, background: '#e0e0e0', outline: 'none',
    cursor: 'pointer',
  }
      ...
      <div style={{
        ...
        <select
          ...
          style={{
            ...
            fontFamily: 'var(--font)', color: 'var(--gray-900)',
      ...
      {selected && result && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--gray-700)', marginBottom: '16px' }}>
              Scenario controls
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              ...
              <div>
                ...
                <input
                  type="range" min={30} max={180} step={5} value={turnaround}
              ...
              <div>
                ...
                <input
                  type="range" min={1} max={Math.max(80, baseTrucks * 2)} step={1} value={trucks}
              ...
              <div>
                ...
                <input
                  type="range" min={10} max={95} step={1} value={utilTarget}
                  onChange={e => setUtilTarget(Number(e.target.value))}
                  style={sliderStyle}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: 'var(--gray-400)', marginTop: '4px' }}>
                  <span>10%</span>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--gray-600)' }}>Price per m³</span>
                  <span style={{ fontSize: '14px', fontWeight: '700', fontFamily: 'var(--mono)', color: priceM3 !== basePrice ? '#2471A3' : 'var(--gray-900)' }}>
                    ${priceM3}
                  </span>
                </div>
                <input
                  type="range" min={Math.max(10, Math.round(basePrice * 0.5))} max={Math.round(basePrice * 2)} step={1} value={priceM3}
          ...
          <div style={{
                ...
                {[
                  {
                    ...
                    scenario: fmtVolume(result.scenarioAnnual),
                    delta: result.incrementalVolume > 0 ? '+' + fmtVolume(result.incrementalVolume) : '—',
                    positive: result.incrementalVolume > 0
                  },
                  {
                    label: 'Revenue upside',
                    baseline: '—',
                    scenario: fmtMoney(result.revenueUpside),
                    delta: result.revenueUpside > 0 ? '+' + fmtMoney(result.revenueUpside) : '—',
                    positive: result.revenueUpside > 0
                  },
                  {
                    label: 'Contribution upside',
                    baseline: '—',
                    scenario: fmtMoney(result.contributionUpside),
                    delta: result.contributionUpside > 0 ? '+' + fmtMoney(result.contributionUpside) : '—',
                  ...
                  <tr key={row.label} style={{ borderBottom: i < 3 ? '1px solid var(--border)' : 'none' }}>
                    ...
                    <td style={{ padding: '10px 12px', fontSize: '14px', fontFamily: 'var(--mono)', color: 'var(--gray-900)', textAlign: 'right', fontWeight: '600' }}>
                      {row.scenario}
                    </td>
                    <td style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '20px' }}>
              {[
                ...
                { label: 'Fleet capacity (effective)', value: fmtVolume(result.effectiveFleetDaily) + '/day', active: result.bottleneck === 'Fleet / Logistics' },
