...
export function buildIssues(r: CalcResult, a: Answers, meta?: { country?: string }): Issue[] {
  ...
  if (r.scores.dispatch !== null && r.scores.dispatch < 65) {
    ...
    issues.push({
      sev: 'red', pin: bn === 'Logistics' || bn === 'Fleet',
      t: `Truck turnaround ${r.ta} min — ${Math.round((r.ta - r.TARGET_TA) / r.TARGET_TA * 100)}% above ${r.TARGET_TA}-min target`,
      action: 'Require site readiness confirmation before trucks depart',
      rec: `Trucks should only depart when the site confirms the pump crew is ready. Closing this gap recovers ${fmt(r.turnaroundLeakMonthly)}/month.`,
      loss: r.turnaroundLeakMonthly,
  ...
  if (a.mix_split === 'Not sure — no visibility on production mix by strength class') {
    issues.push({ sev: 'amber', pin: false, t: 'No visibility on production mix by strength class', action: 'Map monthly volume by strength class: C20, C25, C30, C35+', rec: 'The split between standard and high-strength concrete determines real margin.', loss: 0 })
  }

  if (a.mix_split === 'Mostly standard strength — over 70% is C20 to C30' && r.contrib > 0 && r.contrib < 20) {
    issues.push({ sev: 'amber', pin: false, t: 'Over 70% standard-grade production — margin improvement available', action: 'Get one price quote for C35+ from your top 3 customers', rec: 'C35+ mixes command 15–30% price premium with moderate cement cost increase.', loss: 0 })
  }
  ...
  if (a.silo_days === 'Under 2 days — high supply risk') {
    issues.push({ sev: 'red', pin: false, t: 'Cement stock under 2 days — one missed delivery stops the plant', action: 'Call your cement supplier today — negotiate a standing delivery schedule', rec: 'A single missed delivery at this stock level halts production entirely. Target: 5-day minimum buffer.', loss: 0 })
  }
  if (a.silo_days === '2 to 5 days — tight, supply-sensitive') {
    issues.push({ sev: 'amber', pin: false, t: 'Cement stock 2–5 days — vulnerable to supply disruption', action: 'Increase reorder point — extend to 7+ days before Ramadan and public holidays', rec: 'Irregular delivery schedules create real stoppage risk at this buffer level.', loss: 0 })
  }
  ...
  if (r.availRate < 0.85 && r.operativeTrucks > 0 && r.trucks > 0) {
    ...
    issues.push({
      sev: r.availRate < 0.70 ? 'red' : 'amber', pin: false,
  ...
  if (r.truckBreakdowns > 2 || a.maint_programme === 'Reactive only — trucks are repaired when they break down' || a.maint_programme === 'No maintenance programme') {
    issues.push({
      sev: r.truckBreakdowns > 4 ? 'red' : 'amber', pin: false,
  ...
  if (r.surplusLeakMonthly > 0 && r.surplusMid >= 0.35) {
    issues.push({
      sev: r.surplusMid >= 0.75 ? 'red' : 'amber', pin: false,
  ...
  if (r.rejectPct > 1.5 && a.reject_cause === 'Rejection not tracked — unknown') {
    issues.push({ sev: 'amber', pin: false, t: 'Rejections not tracked — cause unknown, improvement impossible', action: 'Start logging every rejection with cause code from tomorrow', rec: 'A simple cause code on each return ticket costs nothing and reveals the fix within 30 days.', loss: 0 })
  }
  if (r.rejectPct > 1.5 && a.reject_cause === 'Heat and stiffening during transit') {
    issues.push({ sev: 'amber', pin: false, t: 'Heat stiffening causing rejections — retarder dosage needs review', action: 'Increase retarder dosage on loads with transit time over 45 min', rec: 'Retarder adjustment to mix design for long-haul loads is the standard fix.', loss: 0 })
  }
  if (r.rejectPct > 1.5 && a.reject_cause === 'Site not ready — pump or crew unavailable') {
    issues.push({ sev: 'amber', pin: false, t: 'Site unreadiness causing rejections — a dispatch and demurrage problem', action: 'Implement pre-departure site readiness call', rec: 'The fix is in dispatch protocol and contract enforcement.', loss: 0 })
  }
  ...
  if (a.aggregate_days === 'Under 2 days — high supply risk') {
    issues.push({ sev: 'red', pin: false, t: 'Aggregate stock under 2 days — one missed delivery stops the plant', action: 'Call your aggregate supplier today — negotiate a standing delivery schedule', rec: 'Target: 5-day minimum buffer for sand and all aggregate types.', loss: 0 })
  }
  if (a.aggregate_days === '2 to 5 days — tight, supply-sensitive') {
    issues.push({ sev: 'amber', pin: false, t: 'Aggregate stock 2–5 days — vulnerable to supply disruption', action: 'Increase reorder point — extend to 7+ days before Ramadan and public holidays', rec: 'Aggregate supply disruptions are as common as cement disruptions in GCC markets.', loss: 0 })
  }
