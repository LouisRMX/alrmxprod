...
export const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    qs: [
      ...
      {
        id: 'delivery_radius',
        label: 'What is the average one-way delivery distance from plant to active construction sites?',
        hint: 'Benchmark zones: under 5 km (dense urban core), 5-12 km (city), 12-20 km (suburban), over 20 km (regional). If your sites span a range, enter the midpoint or the value representing the majority of your volume.',
        howto: 'Pull up the last 20 delivery tickets or dispatch log. Note the one-way distance to each site and calculate the average, or pick the value that represents the majority of your delivery volume.',
        type: 'num',
        unit: 'km',
        req: true,
        info: {
          what: 'Typical one-way distance to active construction sites, in km.',
      ...
      {
        id: 'batch_cycle_min',
        label: 'What is the actual batch cycle time, from aggregate loading to concrete discharge into the truck drum?',
        hint: 'Supplements the batch cycle dropdown with a measured value. Time one complete batch yourself.',
        type: 'num',
        unit: 'minutes',
        req: false,
        info: {
          what: 'Actual measured batch cycle time.',
          why: 'Precise batch cycle time determines plant throughput ceiling. Dropdown gives a range, this gives the actual.',
          calc: 'Used in value stream analysis as a measured data point. Overrides dropdown midpoint when available.',
        },
      },
      {
        id: 'order_to_dispatch_min',
        label: 'What is the actual time from a confirmed customer order to the first loaded truck leaving the gate?',
        hint: 'Supplements the order-to-dispatch dropdown. Time 3-5 orders yourself and average.',
        type: 'num',
        unit: 'minutes',
        req: false,
        info: {
          what: 'Measured order-to-dispatch time.',
          why: 'The dropdown gives a range. This gives the actual average, which can be surprisingly different from what the dispatcher reports.',
          calc: 'Used in dispatch analysis. Overrides dropdown midpoint when available.',
        },
      },
      {
        id: 'washout_min',
        label: 'How long does the washout process actually take, from truck arrival at the washout bay to drum clean and ready?',
        hint: 'Supplements the washout time dropdown. Time 3-5 washouts yourself.',
        type: 'num',
        unit: 'minutes',
        req: false,
        info: {
          what: 'Measured washout/turnaround time at plant.',
          why: 'Precise washout time fills a blind spot in the turnaround breakdown. Combined with field log timestamps, it separates washout from plant queue.',
          calc: 'Used in value stream analysis. Overrides dropdown midpoint when available.',
        },
      },
    ],
  },
  ...
  },
  {
    ...
    qs: [
      ...
      {
        id: 'plant_idle',
        label: 'Do you experience both queuing AND idle periods on the same day?',
        hint: 'Trucks waiting at the plant or at sites, AND periods where the plant sits idle with no trucks to load. This pattern is the strongest signal for dispatch coordination issues.',
        type: 'text',
        info: {
          what: 'Whether trucks queue and the plant sits idle on the same day.',
          why: 'The clearest dispatch bottleneck signal. Both queuing and idle = cluster dispatching pattern.',
          calc: 'Used in hypothesis ranking: "Yes" with explanation elevates dispatch timing as confirmed primary cause.',
        },
      },
    ],
  },
