...
function scoreStatus(score: number | null): { label: string; color: string } {
  if (score == null) return { label: '', color: 'var(--gray-400)' }
  if (score >= 80) return { label: 'On track',        color: '#2a9d6e' }
  if (score >= 60) return { label: 'Needs attention', color: '#c96a00' }
  return              { label: 'At risk',            color: '#cc3333' }
}
...
export default function OwnerReportView({
  ...
  const boardItems = useMemo(() => {
    if (focusActions?.filter(Boolean).length) {
      return focusActions.filter(Boolean).map(text => ({ text, lossMonthly: 0 }))
    }
    const withAction = issues.filter(i => i.action && i.loss > 0)
    return [
      ...withAction.filter(i => i.category === 'bottleneck').sort((a, b) => b.loss - a.loss),
      ...withAction.filter(i => i.category !== 'bottleneck').sort((a, b) => b.loss - a.loss),
    ].slice(0, 3).map(i => ({ text: i.goal ?? i.action!, lossMonthly: i.loss }))
  }, [issues, focusActions])
  ...
  const scores = [
    { key: 'prod',     label: 'Production', score: calcResult.scores?.prod     ?? null },
    { key: 'dispatch', label: 'Dispatch',   score: calcResult.scores?.dispatch  ?? null },
    { key: 'fleet',    label: 'Fleet',      score: calcResult.scores?.fleet    ?? null },
    { key: 'quality',  label: 'Quality',    score: calcResult.scores?.quality  ?? null },
  ]
        ...
        <div style={{
          ...
          {totalLoss > 0 ? (
            <>
              ...
              <div style={{ fontSize: '12px', color: 'var(--gray-400)', marginTop: '4px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {scores.map(({ key, label, score }) => {
              const { label: statusLabel, color } = scoreStatus(score)
              const isBottleneck = key === bottleneck
              return (
                <div key={key} style={{
                  display: 'flex', flexDirection: 'column', gap: '2px',
                  padding: '8px 12px', borderRadius: '8px',
                  background: isBottleneck ? '#fff8f0' : 'var(--gray-50)',
                  border: isBottleneck ? '1px solid #f5cba0' : '1px solid var(--border)',
                  minWidth: '80px',
                }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                    ...
                    {isBottleneck && (
                      <span style={{ fontSize: '9px', fontWeight: 600, color: '#c96a00', textTransform: 'uppercase' }}>
                        bottleneck
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: '11px', color: 'var(--gray-500)' }}>{label}</div>
                  {statusLabel && (
                    <div style={{ fontSize: '10px', color, fontWeight: 500 }}>{statusLabel}</div>
                  )}
                </div>
              )
            })}
